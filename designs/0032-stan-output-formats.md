...
For file-based output, (i.e., CmdStan), use of an output file path will be replaced by an output directory path,
and this directory will be populated with the following set of output files:
  ...
  + `metric` - the stepsize and inverse mass matrix output at the end of adaptation.
...
Note that if we adjoin all the columns in tables `log_prob`, `algorithm_iter_state`, and `model_sample`, the resulting data table would contain the equivalent information
...
We propose to refactor the core Stan modules, primarily the
`stan::services` and `stan::callbacks` modules, adding new classes as needed.
To manage multiple output components, we will add a set algorithm-specific output handler

Therefore we want to keep the use of writer callbacks by the algorithms.
The use of writer callbacks keeps the inference algorithms output-format agnostic;
CmdStan writer callbacks are C++ output streams; RStan and PyStan use R and Python objects.
