...
Stan already has an informally defined release cycle. This document seeks to write down what
...
### One month before release (three months after last release)

- The release manager decides on precise release date. Typically a Monday, though considerations
...
is the content of the release. During the time before the freeze, the release manager (and
...
Defining a critical issue is a difficult judgment call. Generally, to a given user,
whichever problem they are facing in their modeling will be critical to them. This means
...
Some issues that should always be considered critical are:
...
These descriptions usually need editing for tense and clarity, and are also
generally re-ordered so the most important changes appear first. Small changes,
like updating CI workflows, can often be omitted entirely.

A few important features should be selected for highlighting in the release
announcement.
...
- We could have releases occur without a set schedule. This was used at one point
  in Stan's history, but led to disagreements about what was good enough to be released,
...
and backwards compatibility is not guaranteed for features pre-stabilization.
